...
/// // socket.
/// let sender_socket = RawSocket::bind(&"127.0.0.1:0".parse()?)?;
/// let echoer_socket = RawSocket::bind(&"127.0.0.1:0".parse()?)?;
///
...
impl RawSocket {
    ...
    /// let socket = match RawSocket::bind(&"127.0.0.1:0".parse()?) {
    ...
    /// # }
    /// ```
    // Probably this need to be refactored to be able to pass in a
    // protocol. right now this is only useful for listening to incoming
    // ICMP packets.
    pub fn bind(addr: &SocketAddr) -> std::io::Result<RawSocket> {
        let socket: Socket = match addr {
            // *NOT* setting the protocol will succeed here,
            // but will not return any IPv6 packets.
            // IPv4 appears to work (MacOS tested)
            SocketAddr::V4(_ip) => {
    ...
    pub fn from_socket(socket: socket2::Socket) -> std::io::Result<RawSocket> {
        Ok(RawSocket {
            sys: sys::RawSocket::new(socket)?,
    ...
    /// let socket = RawSocket::bind(&addr)?;
    /// #    Ok(())
    /// # }
    /// #
    /// # fn main() {
    ...
    ///
    /// The returned `RawSocket` is a reference to the same socket that this
    ...
    ///
    /// // We must bind it to an open address.
    /// let socket = RawSocket::bind(&"127.0.0.1:0".parse()?)?;
    ...
    pub fn try_clone(&self) -> std::io::Result<RawSocket> {
        self.sys.try_clone().map(|s| RawSocket {
            sys: s,
    ...
    /// # }
    /// ```
    pub fn recv_from(&self, buf: &mut [u8]) -> std::io::Result<(usize, socket2::SockAddr)> {
    ...
    /// use mio::super::net::RawSocket;
    ///
    /// let broadcast_socket = RawSocket::bind(&"127.0.0.1:0".parse()?)?;
    ...
    #[cfg(all(unix, not(target_os = "fuchsia")))]
    pub fn recv_bufs(&self, bufs: &mut [&mut IoVec]) -> std::io::Result<usize> {
        self.sys.readv(bufs)
    }
    ...
    /// [link]: https://doc.rust-lang.org/nightly/std/io/enum.ErrorKind.html#variant.WouldBlock
    #[cfg(all(unix, not(target_os = "fuchsia")))]
    pub fn send_bufs(&self, bufs: &[&IoVec]) -> std::io::Result<usize> {
