...
use crate::rawsocket::mio::RawSocket as MioRawSocket;
use socket2;

/// A RAW socket.
///
/// After creating a `RawSocket` by [`bind`]ing it to a socket address, data can be [sent to] and
...
///
/// This type is an async version of [`socket2::Socket`].
///
/// [`bind`]: #method.bind
...
/// [`socket2::Socket`]: https://doc.rust-lang.org/std/net/struct.RawSocket.html
///
/// ## Examples
///
/// ```no_run
...
/// #
/// use async_std::net::RawSocket;
///
/// let socket = RawSocket::bind("127.0.0.1:8080").await?;
...
impl RawSocket {
    ...
    /// use async_std::net::RawSocket;
    ///
    /// let socket = RawSocket::bind("127.0.0.1:0").await?;
    /// #
    /// # Ok(()) }) }
    /// ```
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> io::Result<RawSocket> {
        let mut last_err = None;

        for addr in addr.to_socket_addrs().await? {
            match MioRawSocket::bind(&addr) {
                Ok(mio_socket) => {
                    return Ok(RawSocket {
    ...
    /// let socket = RawSocket::bind(&socket_addr).await?;
    ///
...
impl From<socket2::Socket> for RawSocket {
    /// Converts a `socket2::Socket` into its asynchronous equivalent.
    fn from(socket: socket2::Socket) -> RawSocket {
        let mio_socket = MioRawSocket::from_socket(socket).unwrap();
        RawSocket {
            watcher: Watcher::new(mio_socket),
        }
    }
...
cfg_unix! {
    ...
    impl FromRawFd for RawSocket {
        unsafe fn from_raw_fd(fd: RawFd) -> RawSocket {
            socket2::Socket::from_raw_fd(fd).into()
        }
    }
...
cfg_windows! {
    ...
    //     unsafe fn from_raw_socket(handle: RawSocket) -> RawSocket {
    //         net::RawSocket::from_raw_socket(handle).into()
    //     }
    // }
    //
